...
class PacmanAgent(CaptureAgent):
	...
	def establishModel():
		'''
		This method helps the agent to learn the game and the map.
		Mainly aim at finding all dead end.
		'''
		openQ = util.Queue()
		close = [self.initialAgents]
		openQ.push(self.initialAgents)

		while not openQ.isEmpty():
			state = openQ.pop()
			neighbors = Actions.getLegalNeighbors(state, self.walls)

			for nextState in neighbors:
				if nextState in close:
					continue
				close.append(nextState)
				x, y = nextState


	def isWayToDeadEnd(initstate, targetState):
		'''
		This method read two params:
				initState is the current location
				targetState is the state we want to check if it is on a dead way
		Now assume we are at target State:
				we try every possible states we are able to move to (cloest state first, sth like BFS)
		'''
		
		openQ = util.PriorityQueue()
		close = [initstate, targetState]
		openQ.push(targetState, 1)

		while not openQ.isEmpty():
			currState = openQ.pop()
			neighbors = gameState.getLegalNeighbors(currState, self.walls)
			for nextState in neighbors:
				if nextState in close:
					continue
				if nextState == gameState.getAgentPosition(self.index): # ???????
					return False

				close.append(nextState)
				openQ.push(nextState, self.getMazeDistance(nextState, gameState.getAgentPosition(self.index)))
...
class AttackerAgent(PacmanAgent):
	...
	def registerInitialState(self, gameState):
		...
		# return the path from deadend to the gateway (deadend postion, gateWay), the gateway is the target
		self.deadEndPath = []
		...
		while not openList.isEmpty():
			...
			for successor in Actions.getLegalNeighbors(currentState, self.walls):
				...
				if successor in self.deadEnds:
					continue
				g_value = len(path) + 1
				h_value = - 1 * self.getMazeDistance(successor, enemyMonstersPos[0])
				f_value = g_value + h_value
				action = Actions.vectorToDirection((successor[0] - currentState[0], successor[1] - currentState[1]))
				openList.push((successor, path + [action]), f_value)
				closeList.append(successor)
...
class DefenderAgent(PacmanAgent):
	...
	def chooseAction(self, gameState):
		...
		if (len(enemyPacman) == 0):
			...
			if (len(disapperFood) > 0):
					...
					for food in foodList:
						dist = self.getMazeDistance(foodPos, food)
						if (dist < minDis):
								minDis = dist
								closestFood = food              
		...
		#chase enemy
			...
			target = random.choice(enemyPacman)
			actions = self.AStarSearch(gameState,myPos,target)
			self.FoodBefore = self.getFoodYouAreDefending(gameState).asList()
			return actions[0]
		'''
