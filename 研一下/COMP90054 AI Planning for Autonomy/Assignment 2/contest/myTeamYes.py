...
class MyAgent(CaptureAgent):
	...
	'''
	def getCurrentApproximateAttackZone(self):
		if self.red:
			left = 0
			right = self.width/2
		else:
		...
		for i in range(left,right):
			for j in range(self.height):
				...
				if not self.walls[i][j]:
					sum = 0
					for pri in self.inRoundlostFoodLogPosition:
						sum += self.getMazeDistance(dot, pri)
					if sum < dis:
						dis = sum
						zone = dot
	...
	def getMostCloestFood(self, gameState):
		...
		for food in self.foodToEat:
			...
			'''
			distance = self.getMazeDistance(food, position)
		...
		if self.step <= 250:
			...
			enemyPacmanNumberLast = 0
			enemyPacmanLast = []
			enemyGhostLast = []
			oppoScaredTimerLast = 0
			for enemy in self.opponents:
				oppoScaredTimerLast = lastGameState.getAgentState(enemy).scaredTimer
				if lastGameState.getAgentState(enemy).isPacman:
					enemyPacmanNumberLast += 1
					if lastGameState.getAgentPosition(enemy) is not None:
						enemyPacmanLast.append(gameState.getAgentPosition(enemy))
				if not gameState.getAgentState(enemy).isPacman:
					if gameState.getAgentPosition(enemy) is not None:
						enemyGhostLast.append(gameState.getAgentPosition(enemy))

			if len(enemyGhostLast) > 0 and oppoScaredTimerLast <= 0:
				minimum = 9999
				cloestEnemyLast = enemyGhostLast[0]
				for enemyPosition in enemyGhostLast:
					distance = self.getMazeDistance(enemyPosition, position)
					if distance < minimum:
						minimum = distance
						cloestEnemyLast = enemyPosition
				anotherEnemyLast = None
				for each in enemyGhostLast:
						if each != cloestEnemyLast:
							anotherEnemyLast = each
				while i < len(sortedFood):
					# get most cloest food, and the cloest enemy is further than me from this food.
					if self.getMazeDistance(sortedFood[i][0], cloestEnemyLast) <= sortedFood[i][1]:
						i += 1
						continue

					foodX = sortedFood[i][0][0]
					foodY = sortedFood[i][0][1]

					# no another enemy(not the cloest enemy)
					if anotherEnemyLast is not None:
						if foodX in range(anotherEnemyLast[0] - 2, anotherEnemyLast[0] + 3) or foodY in range(anotherEnemyLast[1] - 2, anotherEnemyLast[1] + 3):
							i += 1
							continue
					break
		...
		if len(self.enemyGhost) > 0 and self.oppoScaredTimer <= 0:
			...
			anotherEnemy = None
			for each in self.enemyGhost:
					if each != cloestEnemy:
						anotherEnemy = each

			while i < len(sortedFood):
				# get most cloest food, and the cloest enemy is further than me from this food.
				if self.getMazeDistance(sortedFood[i][0], cloestEnemy) <= sortedFood[i][1]:
					i += 1
					continue
				...
				# no another enemy(not the cloest enemy)
				if anotherEnemy is not None:
					if foodX in range(anotherEnemy[0] - 2, anotherEnemy[0] + 3) or foodY in range(anotherEnemy[1] - 2, anotherEnemy[1] + 3):
						i += 1
						continue
				break
	...
	'''
	def getMostDenseFoodPositionToDefense(self, gameState):
		minDistance = 9999
	...
	'''
	'''
	def goalOfAttackAgentInDefenseMode(self, gameState):
		goal = self.gate3_1
		if len(self.enemyPacman) == 1:
			goal = self.enemyPacman[0]
		if len(self.enemyPacman) == 2:
			if self.getMazeDistance(gameState.getAgentPosition(self.index), self.enemyPacman[0]) < self.getMazeDistance(gameState.getAgentPosition(self.index), self.enemyPacman[1]):
				goal = self.enemyPacman[0]
			else:
				goal = self.enemyPacman[1]

		return goal
	'''
	'''
	def goalOfDefenseAgentInDefenseModeFirst(self, gameState):
		goal = self.gate3_2
		if len(self.enemyPacman) == 1:
			goal = self.enemyPacman[0]
		if len(self.enemyPacman) == 2:
	...
	def getAttackGoal(self, gameState):
		...
		return goal
		'''
		elif len(self.foodToEat) <= 2:
			if gameState.getAgentState(self.index).isPacman:
				mini = 9999
				for gate in self.gate:
					dis = self.getMazeDistance(gate, gameState.getAgentPosition(self.index))
					if dis < mini:
						mini = dis
						goal = gate
				return goal
				...
				self.isAttackAgent = False
				return self.getDefenseGoal(gameState)
				'''

	'''
	def getDefenseGoal(self, gameState):
		self.foodOnMySide = self.getFoodYouAreDefending(gameState).asList()
		goal = self.getMostDenseFoodPositionToDefense(gameState)
		for enemy in self.opponents:
			if gameState.getAgentState(enemy).isPacman:
				if gameState.getAgentPosition(enemy) is not None:
					goal = gameState.getAgentPosition(enemy)
		return goal
	'''
			...
			if curPosition == goal:
				...
				for action in curState.getLegalActions(self.index):
					...
					if successorPosition in self.enemyGhost and self.oppoScaredTimer <= 0:
						continue
					if successorPosition not in traversal:
						...
						if not goal == successorPosition:
							traversal.add(successorPosition)
		print 'aStarBug'
	...
	def attackDecision(self, gameState, goal):
				...
				if len(actions) == 1:
					...
					if len(bestAction) == 1:
						...
						# return the action cloest to the capsule
							...
							if len(self.foodToEat) < 3:  # go home if remain food < 2
								bestChoice = []
								for action in bestAction:
									mini = 9999
									for gate in self.gate:
		...
		'''
		elif len(self.enemyGhost) > 0 and self.oppoScaredTimer <= 0 and self.hideStep > 30:
			self.hideStep += 1
			minDistance = 9999
			for i in self.gate:
				distance = self.getMazeDistance(gameState.getAgentPosition(self.index), i)
				if distance < minDistance:
					minDistance = distance
					goal = i
					'''
					...
					if xMax - xMin <= 3 and yMax - yMin <= 3:
						...
						if status.count(False) >= 4:
							...
							if len(self.enemyGhost) > 0:
								...
								return self.attackDecision(gameState, goal)
			'''
			if not gameState.getAgentState(self.index).isPacman and self.step <= 250:
				self.myselfGhost += 1
			else:
				self.myselfGhost = 0
				self.specialMode = False
				self.getSpecialGoal = None
				self.readyToAttack = False
				self.readyStep = 0
				'''

			goal = self.getAttackGoal(gameState)
			# i = self.getCapsules(gameState)
			# print i
			'''
			if self.myselfGhost >= 15:
				self.specialMode = True
				if self.specialGoal == None:
					self.getSpecialPosition(gameState)
					self.setSpecialGoal(gameState)
				if gameState.getAgentPosition(self.index) == self.specialGoal:
					self.specialGoal = None
		...
		if self.isAttackAgent:
			...
			if self.attackMode:
				if self.isDefenseAgentPacman == True and gameState.getAgentState(self.index).isPacman == False:
					...
					return self.attackDecision(gameState, goal)
		# print 3
		# goal = self.getDefenseGoal(gameState)
			return self.chooseActionForDefenseAgent(gameState)
		'''
		except:
			print 'chooseActionBug'
			print gameState.getAgentPosition(self.index)
			print gameState.getLegalActions(self.index)
			return random.choice(gameState.getLegalActions(self.index))
			'''
